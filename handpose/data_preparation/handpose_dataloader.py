...
from egoego.utils.smpl_to_openpose.mapping import EGOEXO4D_EGOPOSE_HANDPOSE_MAPPINGS
...
class hand_pose_anno_loader:
	...
	def __init__(self, args, split, anno_type):
		...
		self.split = split
		self.num_joints = len(EGOEXO4D_EGOPOSE_HANDPOSE_MAPPINGS)  # Number of joints for single hand
	...
	def load_take_raw_data(
		...
		for frame_idx, curr_frame_anno in anno.items():
			...
			for hand_idx, hand_name in enumerate(HAND_ORDER):
				...
				assert sum(np.isnan(one_hand_filtered_3d_kpts_cam)) == sum(~valid_3d_kpts_flag), f"Missing 3d kpts count mismatch: {sum(np.isnan(one_hand_filtered_3d_kpts_cam))} vs {sum(~valid_3d_kpts_flag)}"
				assert sum(np.isnan(one_hand_filtered_3d_kpts_world)) == sum(~valid_3d_kpts_flag), f"Missing 3d kpts count mismatch: {sum(np.isnan(one_hand_filtered_3d_kpts_world))} vs {sum(~valid_3d_kpts_flag)}"
